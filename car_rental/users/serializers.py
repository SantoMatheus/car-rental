...
class CompanyRegisterInputSerializer(serializers.Serializer):
    legal_representative = serializers.CharField(max_length=11, min_length=11)
...
class CompanyRegisterOutputSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Company
        fields = ['legal_representative', 'register_name', 'company_name', 'cnpj', 'phone_number', 'zip_code',
                  'street_name', 'number', 'city', 'state', 'country']


class OutputCompanyInLegalRepresentativeSearch(serializers.ModelSerializer):
    class Meta:
        model = Company
        fields = '__all__'


class SearchLegalRepresentativeOutputSerializer(serializers.ModelSerializer):
    company = OutputCompanyInLegalRepresentativeSearch(source='companies', many=True)
